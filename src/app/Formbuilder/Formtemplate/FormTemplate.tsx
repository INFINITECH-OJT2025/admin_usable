...
export default function FormTemplate() {
  ...
  const itemsPerPage = 4; // Adjust how many forms to show per page
  ...
  const toggleFormStatus = async (formId: number, currentStatus: string, tableName: string) => {
    ...
    try {
      if (currentStatus === 'enabled') {
        ...
        toast.error(`${tableName.toUpperCase()} is disabled successfully!`, {
            position: "top-right",
            autoClose: 5000,
            hideProgressBar: false,
            closeOnClick: true,
        ...
        setLoadingTemplate(false);
        toast.success(`${tableName.toUpperCase()} is enabled successfully!`, {
          position: "top-right",
          autoClose: 5000,
          hideProgressBar: false,
          closeOnClick: true,
      ...
      <DragDropContext onDragEnd={onDragEnd}>
        <div className="divider">
          <div className="divider-text">
            <h4 className="text-3xl font-semibold text-right text-green-600 mb-6">📄 Form Templates</h4>
          </div>
        </div>
        <div className="row">
                ...
                <div className="row">
                  ...
                  {paginatedForms.length > 0 ? (
                    paginatedForms.map((form) => (
                      <div key={form.id} className="col-md-6 mb-4"> {/* Use form.id as the key */}
                        <div
                          className={`p-4 border rounded h-100 ${form.status === 'enabled' ? 'bg-light-blue' : ''} ${loadingTemplate ? 'blur' : ''}`}
                        >
                              ...
                              <div className="collapse" id={`collapse-${form.id}`}>
                                  ...
                                  <span>
                                    <button
                                      ...
                                      onClick={() => toggleFormStatus(form.id, form.status, form.tableName)}
                                    >
                                      {form.status === 'enabled' ? (
                                        <>
                          ...
                          <div className="card scrollable" style={{ height: '300px', overflowY: 'auto' }}>
